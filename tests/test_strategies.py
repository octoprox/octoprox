...
def sample_proxies() -> list[Proxy]:
    ...
    return [
        Proxy(id="proxy-1", host="proxy1.example.com", port=8080, request_count=10, success_count=9, status=ProxyStatus.HEALTHY, avg_latency_ms=100),
        Proxy(id="proxy-2", host="proxy2.example.com", port=8080, request_count=5, success_count=5, status=ProxyStatus.HEALTHY, avg_latency_ms=50),
        Proxy(id="proxy-3", host="proxy3.example.com", port=8080, request_count=20, success_count=15, status=ProxyStatus.DEGRADED, avg_latency_ms=200),
    ]
...
class TestLeastUsedStrategy:
    ...
    def test_select_with_equal_counts(self):
        strategy = LeastUsedStrategy()
        proxies = [
            Proxy(id="proxy-1", host="p1.example.com", port=8080, request_count=10),
            Proxy(id="proxy-2", host="p2.example.com", port=8080, request_count=10),
        ]
...
class TestHealthBasedStrategy:
    ...
    def test_select_prefers_healthy_proxies(self):
        strategy = HealthBasedStrategy()
        proxies = [
            Proxy(id="healthy", host="h.example.com", port=8080, status=ProxyStatus.HEALTHY, success_count=90, request_count=100, avg_latency_ms=50),
            Proxy(id="unhealthy", host="u.example.com", port=8080, status=ProxyStatus.UNHEALTHY, success_count=10, request_count=100, avg_latency_ms=500),
        ]
    ...
    def test_select_falls_back_to_degraded(self):
        strategy = HealthBasedStrategy()
        proxies = [
            Proxy(id="degraded", host="d.example.com", port=8080, status=ProxyStatus.DEGRADED, success_count=70, request_count=100),
            Proxy(id="unhealthy", host="u.example.com", port=8080, status=ProxyStatus.UNHEALTHY, success_count=10, request_count=100),
        ]
    ...
    def test_select_considers_success_rate(self):
        strategy = HealthBasedStrategy()
        proxies = [
            Proxy(id="high-success", host="h.example.com", port=8080, status=ProxyStatus.HEALTHY, success_count=95, request_count=100, avg_latency_ms=100),
            Proxy(id="low-success", host="l.example.com", port=8080, status=ProxyStatus.HEALTHY, success_count=50, request_count=100, avg_latency_ms=100),
        ]
    ...
    def test_select_considers_latency(self):
        strategy = HealthBasedStrategy()
        proxies = [
            Proxy(id="fast", host="f.example.com", port=8080, status=ProxyStatus.HEALTHY, success_count=90, request_count=100, avg_latency_ms=50),
            Proxy(id="slow", host="s.example.com", port=8080, status=ProxyStatus.HEALTHY, success_count=90, request_count=100, avg_latency_ms=500),
        ]
    ...
    def test_calculate_score(self):
        ...
        good_proxy = Proxy(id="good", host="g.example.com", port=8080, success_count=95, request_count=100, avg_latency_ms=50)

        # Low success rate, high latency = low score
        bad_proxy = Proxy(id="bad", host="b.example.com", port=8080, success_count=30, request_count=100, avg_latency_ms=800)
