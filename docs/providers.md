...
|------|--------------|--------------|
| `session` | One gateway; each slot gets a fresh session id and the vendor rotates the exit IP. A connector can instead run the type with **dynamic sessions** (below). | `host`, `port`, `username`, `password`, `session_id` (`length`, `alphabet`, `prefix`), `session_mode_field` |
...
**Dynamic sessions.** A `session` type provisions a fixed pool of sessions by default: `num_proxies` rows, each with a baked-in session id, and a group per country. Residential and mobile products do not need that. Set the connector field named by `session_mode_field` (default `connector.session_mode`) to `dynamic` and the connector holds exactly **one gateway row** whose credentials are rendered per request instead:

- a client `-sessid-<value>` is hashed, together with the project id, into a session id of the descriptor's `session_id` alphabet and length, so the same value reaches the vendor as the same session on every request and every instance, with no shared state and without exposing the client's string;
- a request without `-sessid-` gets a freshly generated session id, so the vendor rotates the exit every request;
- `-cc-<code>` is rendered into the country placeholder for that request and into the vendor session id, so a client session holds one vendor session per country it asks for and returns to the same exit when it asks for that country again; the connector's country list, when set, is an allow-list (a request for an unlisted country skips the connector) and a request with no country gets one of the listed countries, the same one for the life of a client session and a random one per session-less request; host and port templates that reference the country field are rendered per request too, so regional gateways work;
- `num_proxies` is ignored; the shipped descriptors hide it with `show_when` when dynamic;
- the field named by `exit_sample_field` (default `connector.exit_sample_percent`, 0-100, default 5) is the share of requests without a client session that preflight echoes to observe their exit, see [IP attribution]({{ site.baseurl }}/ip-attribution#preflight).

The gateway row is not health-checked: no single exit speaks for a pool that hands out a new one per request, and a probe would only spend vendor traffic on judging a random exit. The health checker marks it healthy without a request, so it is always routable; a dead credential or blocked gateway host shows up as failed client requests on the connector. Metrics, rate limits and quarantine attach to that one row, which is to say to the connector. Routing does not count rows either: the connector's share of a project's traffic comes from its [weight]({{ site.baseurl }}/routing-strategies#connector-weights), so one gateway row next to a fifty-slot pool takes half the requests at equal weights. IP attribution treats the row differently: its exits belong to sessions, not to the row, so nothing is written on it and its sightings come from preflight (see [IP attribution]({{ site.baseurl }}/ip-attribution#preflight)). The shipped Oxylabs, Bright Data, Decodo, IPRoyal and NetNut descriptors expose the choice as **Sessions: Fixed pool / Dynamic (unlimited)** on their session-based types; a custom descriptor adds a select field with values `pool` and `dynamic` under the key its `session_mode_field` names. Switching an existing pool connector to dynamic removes its slot rows on the next sync and creates the gateway row; sticky bindings to the removed rows rebind on the next request.
