              ...
              {table.getHeaderGroups().map((headerGroup) =>
                headerGroup.headers.map((header) => (
                  <th
                    ...
                    className={`px-3 py-2 text-left text-xs font-semibold text-fg-muted uppercase tracking-wider ${
                      header.column.getCanSort() ? 'cursor-pointer select-none hover:bg-line' : ''
                    }`}
          ...
          <tbody className="divide-y divide-line">
            {table.getRowModel().rows.length > 0 ? (
              table.getRowModel().rows.map((row) => (
                <Fragment key={row.id}>
                  <tr
                    className={`h-10 hover:bg-surface-raised/60 transition-colors ${
                      row.getIsSelected() ? 'bg-primary-soft/40' : ''
                    }`}
                  >
                    {row.getVisibleCells().map((cell) => (
                      <td key={cell.id} className="px-3 h-10 align-middle text-fg">
              ...
              <tr>
                <td colSpan={columns.length} className="px-3 py-8 text-center text-fg-muted">
                  {emptyMessage}
                </td>
              </tr>
            )}
        ...
        <div className="flex items-center justify-between text-sm">
          <div className="flex items-center gap-4 text-fg-muted">
            ...
            <div className="flex items-center gap-2">
              <label htmlFor="pageSize" className="text-fg-muted">Rows:</label>
              <select
                id="pageSize"
                value={pageSize}
                onChange={(e) => table.setPageSize(Number(e.target.value))}
                className="px-2 py-1 border border-line-strong rounded text-sm bg-surface text-fg focus:outline-none focus:ring-1 focus:ring-ring"
              >
          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-1 text-fg-muted">
              ...
              <input
                ...
                className="w-14 px-2 py-1 border border-line-strong rounded text-sm text-center bg-surface text-fg focus:outline-none focus:ring-1 focus:ring-ring"
              />
            ...
            <div className="flex items-center gap-1 ml-2">
              <button
                ...
                className="p-1 rounded hover:bg-surface-raised disabled:opacity-40 disabled:cursor-not-allowed"
                title="First page"
              >
        ...
        <div className="flex items-center gap-1">
          ...
          {hasMultiplePages && (
            <div className="relative group">
              ...
              <div className="absolute left-0 top-full mt-1 bg-surface border border-line-strong rounded shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 whitespace-nowrap">
                <button
                  type="button"
